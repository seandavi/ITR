...
-   [If a line is not a complete R command, R will continue the next
    line with a “+”.]{}
...
-   [If you *do not* know the name of the function or object on which
    you want help:]{}

        help.search('microarray')
        RSiteSearch('microarray')

-   [Many online resources which you will collect over the space of the
    course]{}

[Using Help]{} I strongly recommend using `help(newfunction)` for all
...
-   [Vectors can contain numbers, strings (character data), or logical
    values (TRUE and FALSE)]{}

-   [Vectors cannot contain a mix of types!]{}

[Character Vectors]{} Character vectors are entered with each value
surrounded by single or double quotes; either is acceptable, but they
must match. They are always displayed by R with double quotes.
...
-   [If the operation involves two vectors:]{}
    ...
    -   [Different lengths, but one length a multiple of the other: R
        reuses the shorter vector as needed]{}

    -   [Different lengths, but one length *not* a multiple of the
...
![image](figure/plots-1){width="\maxwidth"}

    plot(x,y,main='My First Plot')

![image](figure/plots-2){width="\maxwidth"}

    # change point type
    plot(x,y,pch=3)

![image](figure/plots-3){width="\maxwidth"}
...
![image](figure/plots-4){width="\maxwidth"}
...
![image](figure/plots2-1){width="\maxwidth"}
...
![image](figure/plots2-2){width="\maxwidth"}
...
![image](figure/plots3-1){width="\maxwidth"}

    # A basic histogram
    hist(z, main="Histogram",
        sub="Random normal")

![image](figure/plots3-2){width="\maxwidth"}

    # A "density" plot
    plot(density(z), main="Density plot",
        sub="Random normal")

![image](figure/plots3-3){width="\maxwidth"}

    # A smaller "bandwidth" to capture more detail
    plot(density(z, adjust=0.5),
      sub="smaller bandwidth")

![image](figure/plots3-4){width="\maxwidth"}
...
-   [Multiple plots on the same page:]{}
    ...
    ![image](figure/mfrow-1){width="\maxwidth"}
